...
{
    protected: //Made these  protected so that can be accessed from the synSolver class - Shetal.
	...
	virtual bool findVSADSDecVar(LiteralIdT &theLit, const CComponentId & superComp); //SS - Made virtual for c2syn
	...
	virtual bool recordRemainingComps();
	virtual bool getComp(const VarIdT &theVar, const CComponentId &superComp,
			viewStateT lookUpCls[], viewStateT lookUpVars[]); //made virtual for c2syn - SS
