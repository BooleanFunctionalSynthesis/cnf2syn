...
SOLVER_StateT CMainSolver::countSAT()
{
	...
	while (true)
	{
       ...
       // cout << " decide has returned false. Backtracking " << endl;  
		res = backTrack();
		if (res == EXIT)
			return SUCCESS;
...
bool CMainSolver::decide()
{
	...
	{
      //  if (decStack.TOS_NextComp().getClauseCount > 0) //Contains only clauses with input variable
       // {
        //    buildDTree (decStack.TOS_NextComp())
...
bool CMainSolver::recordRemainingComps()
{
   ...
   // cout << " In record Remaining Comps in Dsharp Solver " << endl;
