...
void Abc_AigReplace_int( Abc_Aig_t * pMan, Abc_Obj_t * pOld, Abc_Obj_t * pNew, int fUpdateLevel )
{
    ...
    // (the new node can be complemented and can have fanouts)
     //printf (" Replacing  = %d, with = %d ", pOld->Id, pNew->Id);
    assert( !Abc_ObjIsComplement(pOld) );
    //    Abc_AigPrintNode (pOld);
    if ( !( Abc_ObjFanoutNum(pOld) > 0 ))
    {
    //    printf ("UpdateLevel = %d\n " , fUpdateLevel);
     //   printf (" Node has no fanout, id = %d, type = %d\n  ", pOld->Id, pOld->Type );
      //  printf (" Replacing with  id = %d, type = %d\n ", pNew->Id, pNew->Type);
