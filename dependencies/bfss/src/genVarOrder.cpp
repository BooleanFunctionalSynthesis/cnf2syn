...
int main(int argc, char * argv[]) {
	...
	string baseFileName(benchmarkName);
	baseFileName = baseFileName.substr(baseFileName.find_last_of("/") + 1);  //Get the file name;
    if (baseFileName.find (".v") !=  string::npos) //This contains the code for the raw file name;
	    baseFileName.erase(baseFileName.find (".v"), string::npos);
    else
	    baseFileName.erase(baseFileName.find (".aig"), string::npos);
	...
	ofstream ofs (varFileName, ofstream::out);
